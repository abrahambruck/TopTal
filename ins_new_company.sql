--  @ins_new_company.sql
--
create or replace procedure ins_new_company 
IS
...
new_owner_id      BUSINESS_OWNER.BO_OWNER_ID%TYPE;
...
CURSOR GetNewCorp IS
  SELECT TMP_ID, NC_CORP_NAME, NC_START_DATE, NC_WEBSITE, NC_ADDRESS1, NC_ADDRESS2, NC_CITY, NC_STATE_PROV, NC_COUNTRY, NC_POSTAL_CODE, NC_EMAIL, NC_HOME_PHONE, NC_CELL_PHONE, NC_FAX, NC_OWNER_ID
  FROM   TMP_TAXPAYER_CORP;
  
CURSOR GetNewOwner (in_TMP_ID IN NUMBER) IS
  SELECT NO_OWNER_NAME, NO_DOB, NO_ADDRESS1, NO_ADDRESS2, NO_CITY, NO_STATE_PROV, NO_COUNTRY, NO_POSTAL_CODE, NO_EMAIL, NO_HOME_PHONE, NO_CELL_PHONE, NO_FAX, NO_OWNER_ID
  FROM   TMP_BUSINESS_OWNER
  WHERE  TMP_ID      = in_TMP_ID
  ORDER BY NO_OWNER_ID;
...
BEGIN
   ...
   FOR x IN 1 .. 20000
   LOOP
    FETCH GetNewCorp INTO in_TMP_ID, Company_Name, Company_Startdate, Company_Website, Street_Address, Street_Address2, City, State, Country, Zip, eMail, Phone, Mobile, Fax, new_owner_id;
	...
	DBMS_OUTPUT.PUT_LINE('new_owner_id from Corp: '||new_owner_id);
    ...
    values                    (next_cuit,      Company_Name,     Company_Startdate, Company_Website, addr_mstr_id,  new_owner_id);

    OPEN GetNewOwner (in_TMP_ID);
	
	   FOR x IN 1 .. 20000
       LOOP
        FETCH GetNewOwner INTO Owner_Name, Owner_DOB, Street_Address, Street_Address2, City, State, Country, Zip, eMail, Phone, Mobile, Fax, new_owner_id;
        EXIT WHEN GetNewOwner%notfound;
	   DBMS_OUTPUT.PUT_LINE('new_owner_id from Ownr: '||new_owner_id);
	   BEGIN
	   ...
	   EXCEPTION WHEN others THEN
	     NULL;
	   END;		
   DBMS_OUTPUT.PUT_LINE('owner_id from business_owner: '||owner_id);
   DBMS_OUTPUT.PUT_LINE('new_owner_id after bus_owner: '||new_owner_id);
        IF owner_id IS NULL and new_owner_id IS NULL then
           select max(bo_owner_id)+1 into new_owner_id from business_owner;
		--ELSE 
		--   new_owner_id := owner_id;
        END IF;
		select seq_address_master.NEXTVAL into addr_mstr_id from dual;
        DBMS_OUTPUT.PUT_LINE('new_owner_id after ELSE: '||new_owner_id);
        insert into ADDRESS_MASTER (AM_ADDRESS_ID, AM_ADDRESS1,    AM_ADDRESS2,     AM_CITY, AM_STATE_PROV, AM_COUNTRY, AM_POSTAL_CODE, AM_EMAIL, AM_HOME_PHONE, AM_CELL_PHONE, AM_FAX)
        values                     (addr_mstr_id,  Street_Address, Street_Address2, City,    State,         Country,    Zip,            eMail,    Phone,         Mobile,        Fax);

		insert into BUSINESS_OWNER (BO_OWNER_ID, BO_CUIT_NUMBER, BO_TAXPAYER_NUMBER, BO_OWNER_NAME, BO_DOB,    BO_ADDRESS_ID)
        values                     (new_owner_id, next_cuit,     owner_ind_id,       Owner_Name,    Owner_DOB, addr_mstr_id);
